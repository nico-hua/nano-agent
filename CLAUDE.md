...
**Nano Agent** (`nano-agent`) is an educational, from-scratch reimplementation of an Agent system. The goal is clarity, readability, and testability of the key Agent boundaries — not a 1:1 port of a production framework. See `AGENTS.md` for the full development principles and workflow; `docs/DEVELOPMENT_PROGRESS.md` is a dated log of what has been built and what is intentionally left out.
